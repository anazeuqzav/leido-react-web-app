...
const BookDetail: React.FC = () => {
  ...
  // Check if we are viewing a book from the library or a search result
  const isLibraryBook = location.pathname.includes('/library-book/');

  // Function to check if a book already exists in the library by its externalId
  ...
  useEffect(() => {
    const fetchBookDetails = async () => {
      ...
      try {
        if (isLibraryBook) {
          // If it's a book from the library, get the book details from the library
          const userBook = getBookById(bookId);
          if (userBook) {
            setLibraryBook(userBook);

            // Search the book in Open Library by title and author (only in English)
            ...
            if (searchData.docs && searchData.docs.length > 0) {
              // Use the first result that matches better
              ...
              // If not found in Open Library, use the library book data
              setCoverUrl(userBook.cover || '');
              setAuthorNames([userBook.author]);

              // Create a BookDetails object with available data
              setBook({
                key: userBook.id,
          ...
          // If it's a search book, get the details normally
          ...
          if (existingBookInLibrary) {
            // Instead of showing the yellow panel, automatically redirect
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-teal-100">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-2/3">
            ...
            {/* Section of buttons and actions */}
                    ...
                    <div className="mt-3 space-y-2">
                      {libraryBook.startDate && (
                        <p className="text-sm flex items-center">
                          <span className="font-medium min-w-[130px]">Reading start date:</span>
                          <span>{new Date(libraryBook.startDate).toLocaleDateString()}</span>
                        </p>
                      )}
                      {libraryBook.readDate && (
                        <p className="text-sm flex items-center">
                          <span className="font-medium min-w-[130px]">Reading end date:</span>
