    ...
    <div className="container mx-auto px-4 py-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div className="flex items-center mb-4 md:mb-0">
          <h2 className="text-2xl font-bold text-teal-800">My Favorite Books</h2>
      ...
      <div className="bg-pink-50 p-4 rounded-lg shadow-sm mb-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-center">
          <div className="bg-white p-3 rounded-md shadow-sm">
            <p className="text-sm text-gray-500">Favorite Books</p>
            <p className="text-2xl font-bold text-teal-700">{favoriteBooks.length}</p>
          </div>
          <div className="bg-white p-3 rounded-md shadow-sm">
            <p className="text-sm text-gray-500">Top Genre</p>
            <p className="text-lg font-bold text-teal-700 truncate">
              {favoriteBooks.length > 0
                ? Object.entries(favoriteBooks.reduce((genres, book) => {
                  if (book.genre) {
          ...
          <div className="bg-white p-3 rounded-md shadow-sm">
            ...
            <p className="text-lg font-bold text-teal-700 truncate">
              {favoriteBooks.length > 0
                ? Object.entries(favoriteBooks.reduce((authors, book) => {
      ...
      {sortedBooks.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-6 text-center">
          {searchTerm ? (
            <p className="text-gray-600">No favorite books match your search criteria.</p>
          ) : (
            <p className="text-gray-600">You haven't added any favorite books yet. Rate books with 5 stars to see them here!</p>
          )}
        </div>
      ) : (
